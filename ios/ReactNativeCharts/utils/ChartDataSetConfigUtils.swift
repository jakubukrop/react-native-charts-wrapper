    ...
    static func commonConfig(_ dataSet: ChartDataSet, config: JSON) {
        ...
        if valueFormatter.string != nil {
            if "largeValue" == valueFormatter.stringValue {
                ...
                let timeUnitCount = config["timeUnitCount"].int != nil ? config["timeUnitCount"].intValue : 1
                dataSet.valueFormatter = CustomChartDateFormatter(pattern: valueFormatterPattern, since: since, timeUnit: timeUnit, timeUnitCount: timeUnitCount);
            } else {
